...
$query = "
    SELECT 
        ...
        s.preco 
    FROM servico s
    INNER JOIN profissionais pr ON s.id_Profissional = pr.id_Profissional
";

$resultado = $con->query($query)->fetch_all(MYSQLI_ASSOC);

$profissionais = [];
foreach ($resultado as $linha) {
    $nome = $linha['nome_Profissional'];
    if (!isset($profissionais[$nome])) {
        $profissionais[$nome] = [];
    }
    $profissionais[$nome][] = [
...
<head>
    ...
    <title>Ic's Hair - Profissionais</title>
...
<body>
    <div class="container-fluid"><br>
        ...
        <div class="text-center mb-4">
            <h2 class="fw-bold btn-custom">Nossos Profissionais</h2>
        </div>

        <div class="container">
            <div class="profissional-container">
                <?php foreach ($profissionais as $nome => $servicos): ?>
                    <div class="profissional-box">
                        <h4><?= htmlspecialchars($nome) ?></h4>
                        <ul class="servico-list">
                            <?php foreach ($servicos as $item): ?>
                                <li><?= htmlspecialchars($item['servico']) ?> — R$ <?= number_format($item['preco'], 2, ',', '.') ?></li>
                            <?php endforeach; ?>
                        </ul>
                    </div>
                <?php endforeach; ?>
